...
impl Default for ControlsPage {
    fn default() -> Self {
        ...
        let mut checks = [
            CheckboxState::new(CheckState::Off),
            CheckboxState::new(CheckState::On),
            CheckboxState::new(CheckState::Indeterminate),
            CheckboxState::new(CheckState::On),
        ];
        ...
        ControlsPage {
            ...
            switches: [
                SwitchState::new(true),
                SwitchState::new(false),
                SwitchState::new(true),
            ],
            switch_styles: [
                SwitchState::new(true),
...
impl Page for ControlsPage {
    ...
    fn draw(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut Ctx) {
        ...
        let [buttons_a, toggles_a, sliders_a] = Layout::vertical([
            Constraint::Length(13),
            Constraint::Length(toggles_h),
            Constraint::Fill(1),
        ])
        .areas(inner);
        ...
        let b = pad(
            card(
                ...
                buttons_a,
                &th,
                &format!("Buttons  ·  {} presses", self.presses),
            ),
            1,
            0,
        );
        ...
        if y + 3 <= b.bottom() {
            ...
            Button::new("Save")
                ...
                .min_width(bw + 4)
                .focused(f(Id::Icon, &self.focus))
                .now(now)
                .theme(&th)
                .render(icon_r, buf, &mut self.icon_btn);
            let wide_r = Rect {
                x: icon_r.right() + 1,
                y,
                width: b.right().saturating_sub(icon_r.right() + 1),
                height: 3,
            };
            Button::new("Full-width button (flat)")
                .style(ButtonStyle::Flat)
                .full_width(true)
                .focused(f(Id::Wide, &self.focus))
                .now(now)
                .theme(&th)
                .render(wide_r, buf, &mut self.wide_btn);
        }
        // ── Toggles (styles) ──
        ...
        for (i, label) in labels.iter().enumerate() {
            ...
            if room(r) {
                Checkbox::new(*label)
                    .tri_state(i == 2)
                    .enabled(i != 3)
                    .focused(f(Id::Check(i), &self.focus))
                    .theme(&th)
                    .render(r, buf, &mut self.checks[i]);
            }
        }
        let check_styles = [
            (CheckStyle::Pill, "Pill"),
            (CheckStyle::Bracket, "Bracket"),
            (CheckStyle::Box, "Box"),
        ...
        for (i, (style, name)) in check_styles.iter().enumerate() {
            ...
            if room(r) {
                Checkbox::new(*name)
                    .style(*style)
                    .focused(f(Id::CheckStyle(i), &self.focus))
                    .theme(&th)
                    .render(r, buf, &mut self.check_styles[i]);
            }
        }

        // c2: switches in every SwitchStyle, then two radio styles side by side
        let switch_styles = [
            (SwitchStyle::Pill, "Dark mode"),
            (SwitchStyle::Slim, "Telemetry"),
            (SwitchStyle::Line, "Line"),
            (SwitchStyle::Round, "Round"),
            (SwitchStyle::Text, "Text"),
            (SwitchStyle::Check, "Check"),
        ];
        ...
        for (i, (style, label)) in switch_styles.iter().enumerate() {
            ...
            let (state, id) = if i < 3 {
                (&mut self.switches[i], Id::Switch(i))
            } else {
                (&mut self.switch_styles[i], Id::SwitchStyle(i))
            };
            Switch::new()
                .label(*label)
                .compact(true)
        ...
        if room(Rect {
            ...
            put(
                ...
                c2.x,
                radios_y,
                "RadioStyle: Bracket · Arrow",
                c2.width,
                muted,
            );
            let half = c2.width / 2;
            RadioGroup::new(vec!["One".into(), "Two".into(), "Three".into()])
                .style(RadioStyle::Bracket)
                .focused(f(Id::RadioBracket, &self.focus))
                .theme(&th)
                .render(
                    Rect {
                        x: c2.x,
                    ...
                    buf,
                    &mut self.radio_bracket,
                );
            RadioGroup::new(vec!["Left".into(), "Center".into(), "Right".into()])
                .style(RadioStyle::Arrow)
                .focused(f(Id::RadioArrow, &self.focus))
                .theme(&th)
                .render(
                    Rect {
                        x: c2.x + half,
        ...
        let rg = Rect {
            y: c3.y,
            height: 6.min(c3.height),
            width: c3.width.saturating_sub(2),
            x: c3.x,
        };
        RadioGroup::new(vec![
            "Small".into(),
        ...
        if room(Rect {
            ...
            RadioGroup::new(vec!["Red".into(), "Green".into(), "Blue".into()])
                .horizontal(true)
                .focused(f(Id::RadioH, &self.focus))
                .theme(&th)
                .render(
                    Rect {
                        y: y3 + 1,
        ...
        if room(Rect {
            ...
            put(
                ...
                c3.x,
                y3,
                "Segmented: Filled · Outline · Underline",
                c3.width,
                muted,
            );
            Segmented::new(seg_items())
                .focused(f(Id::Segmented, &self.focus))
                .theme(&th)
                .render(
                    Rect {
                        y: y3 + 1,
        ...
        if room(Rect {
            ...
            Segmented::new(seg_items())
                .style(SegmentedStyle::Outline)
                .focused(f(Id::SegmentedOutline, &self.focus))
                .theme(&th)
                .render(
                    Rect {
                        y: y3,
        ...
        if room(Rect {
            ...
            Segmented::new(seg_items())
                .style(SegmentedStyle::Underline)
                .focused(f(Id::SegmentedUnderline, &self.focus))
                .theme(&th)
                .render(
                    Rect {
                        y: y3,
                    ...
                    buf,
                    &mut self.segmented_underline,
                );
        }

        // c4: checklist (bracket style) + state summary
        let [c4a, c4b] =
            Layout::horizontal([Constraint::Length(24), Constraint::Fill(1)]).areas(c4);
        put(buf, c4a.x, c4a.y, "CheckList", c4a.width, muted);
        let cl = Rect {
            y: c4a.y + 1,
            height: c4a.height.saturating_sub(1).min(7),
            width: c4a.width.saturating_sub(2),
            x: c4a.x,
        };
        CheckList::new(
            [
        ...
        let summary = [
            format!(
                "checks: {:?}",
                self.checks.iter().map(|c| c.value).collect::<Vec<_>>()
            ),
            format!(
                "switches: {} {} {}",
                self.switches[0].on, self.switches[1].on, self.switches[2].on
            ),
            format!(
                "size: {:?}  colour: {:?}",
                self.radio.selected, self.radio_h.selected
            ),
            format!(
                "segment: {}",
                ["Day", "Week", "Month"][self.segmented.selected.min(2)]
            ),
            ...
            for (i, line) in summary.iter().enumerate() {
                if c4.y + 1 + (i as u16) < c4.bottom() {
                    put(
                        ...
                        c4.x,
                        c4.y + 1 + i as u16,
                        &truncate(line, c4.width as usize),
                        c4.width,
        ...
        // ── Sliders & steppers ──
        let s = pad(
            card(buf, sliders_a, &th, "Sliders, steppers & rating"),
            1,
            0,
        );
        let [s1, s2] =
            Layout::horizontal([Constraint::Percentage(50), Constraint::Fill(1)]).areas(s);
        // 2 rows per slider (track + label); rows never spill onto the card's bottom border
        let row = |a: Rect, i: u16, h: u16| Rect {
            y: a.y + i,
            height: h.min(a.height.saturating_sub(i)),
        ...
        Slider::new()
            ...
            .format(|v| format!("{v:.0}"))
            .ticks(true)
            .focused(f(Id::Volume, &self.focus))
            .duration(ctx.dur(150))
            .now(now)
            .theme(&th)
            .render(row(s1, 0, 2), buf, &mut self.volume);
        Slider::new()
            .label("Brightness")
            .show_value(true)
        ...
        RangeSlider::new()
            ...
            .focused(f(Id::Range, &self.focus))
            .now(now)
            .theme(&th)
            .render(row(s2, 0, 2), buf, &mut self.range);
        let sr = row(s2, 3, 1);
        if sr.bottom() <= s.bottom() {
            ...
            Stepper::new()
                .focused(f(Id::Stepper, &self.focus))
                .theme(&th)
                .render(
                    Rect {
                        x: sr.x + 11,
            ...
            Rating::new()
                .max(5)
                .focused(f(Id::Rating, &self.focus))
                .theme(&th)
                .render(
                    Rect {
                        x: sr.x + 42,
        ...
        if sv.bottom() <= s.bottom() {
            ...
            put(
                ...
                sv.x,
                sv.y,
                &truncate(&line, sv.width as usize),
                sv.width,
                st(th.text_muted, th.background),
            );
        }
    ...
    fn event(&mut self, ev: &Event, ctx: &mut Ctx) -> Outcome {
        match ev {
            ...
            Event::Mouse(m) => {
                ...
                for (i, (button, button_spec)) in
                    self.buttons.iter_mut().zip(BUTTONS.iter()).enumerate()
                {
