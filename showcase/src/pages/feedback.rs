...
const CORNERS: [ToastCorner; 4] = [ToastCorner::TopRight, ToastCorner::BottomRight, ToastCorner::TopLeft, ToastCorner::BottomLeft];
...
impl Default for FeedbackPage {
    fn default() -> Self {
        ...
        FeedbackPage {
            focus: Focus::new([Id::Info, Id::Success, Id::Warning, Id::Error, Id::Corner, Id::Confirm, Id::Alert, Id::Prompt, Id::Sheet, Id::Steps]),
...
impl FeedbackPage {
    fn modal_builder(&self, th: &Theme, now: Instant) -> Modal {
        let m = match self.modal_kind {
            0 => Modal::confirm("Discard changes?", "You have unsaved edits in 3 files. Discarding cannot be undone.")
                .buttons(&[("Keep editing", Variant::Default), ("Discard", Variant::Error)]),
            1 => Modal::alert("Update available", "tuiforge 0.2.0 adds a DataGrid and virtual lists. Restart to apply.").icon("↑"),
            2 => Modal::prompt("Rename branch", "New name for `feature/toasts`:"),
            _ => Modal::new("Sheet").body("A bottom sheet slides up from the edge and keeps the page context visible.").buttons(&[("Close", Variant::Primary)]).kind(ModalKind::Sheet),
        };
        m.theme(th).now(now)
    }
...
impl Page for FeedbackPage {
    ...
    fn bindings(&self) -> &'static [(&'static str, &'static str)] {
        &[("Tab", "Focus"), ("Enter", "Press"), ("1-4", "Toast"), ("Esc", "Dismiss")]
    }
    ...
    fn draw(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut Ctx) {
        ...
        if self.next_cycle.is_none_or(|t| now >= t) {
            let target = if self.animated.value(now) > 0.85 { 0.05 } else { (self.animated.value(now) + 0.3 + (ctx.elapsed() * 7.0).sin().abs() * 0.4).min(1.0) };
        ...
        let [top, mid, bottom] = Layout::vertical([Constraint::Length(12), Constraint::Length(7), Constraint::Fill(1)]).areas(inner);
        let [prog_a, spin_a] = Layout::horizontal([Constraint::Percentage(50), Constraint::Fill(1)]).areas(top);

        // ── progress ──
        let p = pad(card(buf, prog_a, &th, "Progress"), 1, 0);
        let rows: Vec<Rect> = (0..5).map(|i| Rect { y: p.y + i * 2, height: 1, ..p }).filter(|r| r.bottom() <= p.bottom()).collect();
        let labels = ["animated", "indeterminate", "complete", "threshold", "compact"];
        for (i, r) in rows.iter().enumerate() {
            put(buf, r.x, r.y, labels[i], 13, st(th.text_muted, th.background));
            let bar = Rect { x: r.x + 14, width: r.width.saturating_sub(14), ..*r };
            match i {
                0 => ProgressBar::new().show_eta(true).now(now).theme(&th).render(bar, buf, &mut self.animated),
                1 => ProgressBar::new().indeterminate().now(now).theme(&th).render(bar, buf, &mut self.indeterminate),
                2 => ProgressBar::new().variant(Variant::Success).now(now).theme(&th).render(bar, buf, &mut self.complete),
                3 => {
                    let v = self.threshold.value(now);
                    let variant = if v > 0.9 { Variant::Error } else if v > 0.7 { Variant::Warning } else { Variant::Primary };
                    ProgressBar::new().variant(variant).show_eta(false).now(now).theme(&th).render(bar, buf, &mut self.threshold)
                }
                _ => ProgressBar::new().compact(true).show_eta(false).show_percentage(false).now(now).theme(&th).render(bar, buf, &mut self.compact),
            }
        }
        if p.height > 10 {
            let y = p.y + 10;
            ...
            let sp = Rect { x: p.x + 14, y, width: 12, height: 1 };
            StepProgress::new(6).current(self.step).theme(&th).render(sp, buf);
            let hint = format!("  {}/6  ←→ {}", self.step, if focused { "(focused)" } else { "" });
        ...
        let [l1, l2, l3] = Layout::horizontal([Constraint::Percentage(30), Constraint::Percentage(40), Constraint::Fill(1)]).areas(l);
        put(buf, l1.x, l1.y, "LoadingIndicator", l1.width, st(th.text_muted, th.background));
        LoadingIndicator::new().elapsed(ctx.elapsed()).theme(&th).render(Rect { y: l1.y + 1, height: 1, ..l1 }, buf);
        put(buf, l1.x, l1.y + 3, "Blinker", l1.width, st(th.text_muted, th.background));
        Blinker::new().elapsed(ctx.elapsed()).theme(&th).render(Rect { y: l1.y + 3, x: l1.x + 9, width: 2, height: 1 }, buf);
        put(buf, l2.x, l2.y, "Skeleton (card)", l2.width, st(th.text_muted, th.background));
        Skeleton::new().shape(SkeletonShape::Card).elapsed(ctx.elapsed()).theme(&th).render(Rect { y: l2.y + 1, height: l2.height.saturating_sub(1), width: l2.width.saturating_sub(2), ..l2 }, buf);
        put(buf, l3.x, l3.y, "Marquee", l3.width, st(th.text_muted, th.background));
        Marquee::new("tuiforge ships 70+ widgets  •  press 1-4 for toasts  •  m for a dialog  •  ")
            .speed(12.0)
            .elapsed(ctx.elapsed())
            .theme(&th)
            .render(Rect { y: l3.y + 1, height: 1, ..l3 }, buf);
        Skeleton::new().shape(SkeletonShape::Text).lines(&[l3.width.saturating_sub(4), l3.width / 2]).elapsed(ctx.elapsed()).theme(&th).render(Rect { y: l3.y + 3, height: 2, ..l3 }, buf);

        // ── toasts & dialogs | callouts ──
        let [tc, cc] = Layout::horizontal([Constraint::Percentage(55), Constraint::Fill(1)]).areas(bottom);
        let t = pad(card(buf, tc, &th, "Toasts & dialogs"), 1, 0);
        let bw = ((t.width.saturating_sub(3)) / 4).clamp(8, 14);
        let mk = |i: u16| Rect { x: t.x + i * (bw + 1), y: t.y, width: bw, height: 3 };
        let specs = [("Info", Variant::Default, Id::Info), ("Success", Variant::Success, Id::Success), ("Warning", Variant::Warning, Id::Warning), ("Error", Variant::Error, Id::Error)];
        for (i, (label, v, id)) in specs.iter().enumerate() {
            Button::new(*label).variant(*v).min_width(bw).focused(self.focus.is(*id)).now(now).theme(&th).render(mk(i as u16), buf, &mut self.buttons[i]);
        }
        let cy = t.y + 4;
        put(buf, t.x, cy, "corner", 7, st(th.text_muted, th.background));
        let seg = Rect { x: t.x + 8, y: cy, width: t.width.saturating_sub(8), height: 1 };
        Segmented::new(vec!["↗ TR".into(), "↘ BR".into(), "↖ TL".into(), "↙ BL".into()])
            .focused(self.focus.is(Id::Corner))
            .theme(&th)
            .render(seg, buf, &mut self.corner);
        let by = cy + 2;
        let dspecs = [("Confirm", Id::Confirm), ("Alert", Id::Alert), ("Prompt", Id::Prompt), ("Sheet", Id::Sheet)];
        for (i, (label, id)) in dspecs.iter().enumerate() {
            let r = Rect { y: by, ..mk(i as u16) };
            if r.bottom() <= t.bottom() {
                Button::new(*label).style(ButtonStyle::Outline).min_width(bw).focused(self.focus.is(*id)).now(now).theme(&th).render(r, buf, &mut self.buttons[4 + i]);
            }
        }
        ...
        ToastStack::new().theme(&th).now(now).render(Rect { y: t.y + 4, height: t.height.saturating_sub(5), ..t }, buf, &mut self.toaster);
        {
            let hint = "toasts appear in this box — click one to dismiss, Esc clears";
            put(buf, t.x, t.bottom().saturating_sub(1), hint, t.width, st(th.text_disabled, th.background));
        }

        let c = pad(card(buf, cc, &th, "Callouts (click the first to dismiss)"), 1, 0);
        let cspecs = [
            ("Heads up", "Callouts are inline, non-blocking notes.", Variant::Primary, CalloutBorder::LeftBar, true),
            ("Deployed", "Build 4821 is live in all regions.", Variant::Success, CalloutBorder::Round, false),
            ("Quota", "You have used 92% of your monthly minutes.", Variant::Warning, CalloutBorder::LeftBar, false),
            ("Failed", "Payment declined: card expired.", Variant::Error, CalloutBorder::Round, false),
        ];
        let mut y = c.y;
        for (i, (title, msg, v, border, dismiss)) in cspecs.iter().enumerate() {
            ...
            let r = Rect { x: c.x, y, width: c.width, height: h };
            Callout::new(*title, *msg).variant(*v).border_style(*border).dismissible(*dismiss).theme(&th).render(r, buf, &mut self.callouts[i]);
    ...
    fn event(&mut self, ev: &Event, ctx: &mut Ctx) -> Outcome {
        ...
        let toast = |t: &mut Toaster, i: usize| match i {
            0 => t.push(Toast::new("Info", "Nightly build finished in 4m 12s.").timeout(Duration::from_secs(4)).progress(true)),
            1 => t.success("Settings saved to ~/.config/app.toml"),
            2 => t.push(Toast::new("Warning", "Disk usage at 91% on /var").variant(Variant::Warning).timeout(Duration::from_secs(6)).progress(true)),
            _ => t.push(Toast::new("Error", "Connection to api.example.com timed out").variant(Variant::Error).no_timeout()),
        };
        match ev {
            Event::Key(k) => {
