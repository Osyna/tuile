...
impl InputsPage {
    fn new() -> Self {
        ...
        let mut s = Self {
            ...
            tags: MultiSelectState::new(&[
                "Frontend", "Backend", "DevOps", "ML", "Mobile", "Database",
            ]),
            editor: TextAreaState::with_text(
                "fn fibonacci(n: usize) -> usize {\n    match n {\n        0 => 0,\n        1 => 1,\n        _ => fibonacci(n - 1) + fibonacci(n - 2),\n    }\n}\n\nfn main() {\n    for i in 0..10 {\n        println!(\"fib({}) = {}\", i, fibonacci(i));\n    }\n}",
            ),
            readonly: TextAreaState::with_text(
...
impl Page for InputsPage {
    ...
    fn draw(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut Ctx) {
        ...
        // Bottom: editors
        let [top_area, editor_area] =
            Layout::vertical([Constraint::Length(22), Constraint::Fill(1)]).areas(area);
        let [form_area, controls_area] =
    ...
    fn event(&mut self, ev: &Event, ctx: &mut Ctx) -> Outcome {
        ...
        {
            let in_country =
                mouse_in(self.country.hit.area, m) || mouse_in(self.country.dropdown_area, m);
            let in_language =
                mouse_in(self.language.hit.area, m) || mouse_in(self.language.dropdown_area, m);
...
impl InputsPage {
    ...
    fn draw_editors(&mut self, area: Rect, buf: &mut Buffer, th: &Theme, now: Instant) {
        ...
        let title_style = st(th.text, bg).add_modifier(Modifier::BOLD);
        let editor_inner = Border::Round.draw_titled_with(
            buf,
            editor_card,
            th.border_blurred,
            bg,
            " Editor (line numbers, syntax) ",
            Alignment::Left,
            title_style,
        );
        ...
        if editor_inner.height >= 3 {
            let text_area = Rect {
                ...
                height: editor_inner.height.saturating_sub(1),
                ..editor_inner
            };
            TextArea::new()
                .line_numbers(true)
        ...
        let readonly_inner = Border::Round.draw_titled_with(
            buf,
            readonly_card,
            th.border_blurred,
            bg,
        ...
        if readonly_inner.height >= 3 {
            let text_area = Rect {
                width: readonly_inner.width,
                height: readonly_inner.height.saturating_sub(1),
                ..readonly_inner
            };
            TextArea::new().read_only(true).now(now).theme(th).render(
                text_area,
                buf,
                &mut self.readonly,
            );
...
fn highlight_rust(line: &str) -> Vec<(usize, usize, Style)> {
    let keywords = [
        "fn", "let", "mut", "pub", "use", "struct", "impl", "for", "in", "return", "if", "else",
        "match", "usize",
    ];
