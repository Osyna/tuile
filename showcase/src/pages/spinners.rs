...
impl SpinnersPage {
    fn groups(&self) -> [(&'static str, Vec<&'static SpinnerDef>); 2] {
        ...
        let pick = |set: &[&'static SpinnerDef]| -> Vec<&'static SpinnerDef> {
            set.iter().copied().filter(|d| q.is_empty() || d.name.to_ascii_lowercase().contains(&q)).collect()
        };
        [("tuiforge originals", pick(spinners::ORIGINALS)), ("yaspin / cli-spinners", pick(spinners::YASPIN))]
    }
    ...
    fn step(&mut self, delta: i32) {
        ...
        let cur = order.iter().position(|d| std::ptr::eq(*d, self.selected)).unwrap_or(0) as i32;
    ...
    fn scroll_to_selected(&mut self) {
        ...
        let row = rows.iter().position(|r| matches!(r, Row::Cells(c) if c.iter().any(|d| std::ptr::eq(*d, self.selected)))).unwrap_or(0);
        self.scroll = keep_visible(self.scroll, row, self.rows_visible.max(1));
    }

    fn snippet(&self) -> String {
        format!("Spinner::new(&spinners::{}).label(\"Working…\").now(now)", const_name(self.selected.name))
    }
...
impl Page for SpinnersPage {
    ...
    fn draw(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut Ctx) {
        ...
        let [head, grid_a, preview_a] = Layout::vertical([Constraint::Length(1), Constraint::Fill(1), Constraint::Length(preview_h)]).areas(inner);

        // filter row
        let count = self.order().len();
        let counter = format!("{count} / {}", spinners::ALL.len());
        put_right(buf, head, &counter, st(th.text_muted, th.background));
        let filter_area = Rect { width: head.width.saturating_sub(counter.len() as u16 + 2).min(40), ..head };
        Input::new().placeholder("Type to filter…").prefix("/ ").compact(true).focused(true).theme(&th).render(filter_area, buf, &mut self.filter);

        // grid
        let g = card(buf, grid_a, &th, "Catalog (↑↓←→ select · Enter for the snippet · hover to preview)");
        let g = pad(g, 1, 0);
        ...
        self.hits.clear();
        for (i, row) in rows.iter().enumerate().skip(self.scroll).take(g.height as usize) {
            let y = g.y + (i - self.scroll) as u16;
            match row {
                Row::Header(t) => {
                    put(buf, g.x, y, t, g.width, st(th.text_muted, th.background).add_modifier(Modifier::BOLD));
                }
                Row::Cells(defs) => {
                    for (c, def) in defs.iter().enumerate() {
                        let rect = Rect { x: g.x + c as u16 * cw, y, width: cw.saturating_sub(1), height: 1 };
                        let is_sel = std::ptr::eq(*def, self.selected);
                        let is_hover = self.hover.is_some_and(|h| std::ptr::eq(h, *def));
                        let bg = if is_sel { th.cursor_bg } else if is_hover { th.hover_bg } else { th.background };
                        fill(buf, rect, bg);
                        let slot = def.width().min(rect.width);
                        let (glyph_fg, name_fg) = if is_sel { (th.cursor_fg, th.cursor_fg) } else { (th.primary, th.text) };
                        put(buf, rect.x, y, def.frame(since(now)), slot, st(glyph_fg, bg));
                        let nx = rect.x + slot + 1;
                        if nx < rect.right() {
                            put(buf, nx, y, &truncate(def.name, (rect.right() - nx) as usize), rect.right() - nx, st(name_fg, bg));
                        }
        ...
        if rows.len() > g.height as usize {
            let sb = Rect { x: g.right(), y: g.y, width: 1, height: g.height };
            Scrollbar::vertical(rows.len(), g.height as usize).offset(self.scroll).theme(&th).render(sb, buf, &mut self.scrollbar);
        }

        // preview
        if preview_h > 0 {
            let def = self.hover.unwrap_or(self.selected);
            let p = pad(card(buf, preview_a, &th, &format!("Preview · spinners::{}", const_name(def.name))), 1, 0);
            let meta = format!("{} frames · {} ms · {} cell{} wide", def.frames.len(), def.interval_ms, def.width(), if def.width() == 1 { "" } else { "s" });
            put(buf, p.x, p.y, &meta, p.width, st(th.text_muted, th.background));
            Spinner::new(def).label("Working…").now(now).theme(&th).render(Rect { y: p.y + 1, height: 1, ..p }, buf);
            let frames = def.frames.join(" ");
            put(buf, p.x, p.y + 2, &truncate(&frames, p.width as usize), p.width, st(th.text_muted, th.background));
            put(buf, p.x, p.y + 3, &truncate(&self.snippet(), p.width as usize), p.width, st(th.text_accent, th.background));
        }
    }
    ...
    fn event(&mut self, ev: &Event, ctx: &mut Ctx) -> Outcome {
        match ev {
            Event::Key(k) if is_press(k) => match k.code {
                ...
                KeyCode::Right if k.modifiers.contains(KeyModifiers::ALT) || self.filter.value().is_empty() => {
                    self.step(1);
                    Outcome::Consumed
                }
                KeyCode::Left if k.modifiers.contains(KeyModifiers::ALT) || self.filter.value().is_empty() => {
                    self.step(-1);
                    Outcome::Consumed
                }
                KeyCode::PageDown => {
            ...
            Event::Mouse(m) => {
                ...
                let pos = mouse_pos(m);
                self.hover = self.hits.iter().find(|(r, _)| r.contains(pos)).map(|(_, d)| *d);
                if is_left_down(m)
