...
impl Default for ThemesPage {
    fn default() -> Self {
        Self { cursor: 0, hover: None, list_hits: Vec::new(), hue_shift: 0.0, slider: Rect::default(), dragging: false }
    }
}
...
impl Page for ThemesPage {
    ...
    fn draw(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut Ctx) {
        ...
        let [list_a, right] = Layout::horizontal([Constraint::Length(26), Constraint::Fill(1)]).areas(inner);
        ...
        for (i, spec) in BUILTIN.iter().enumerate() {
            ...
            let row = Rect { x: list_in.x, y, width: list_in.width, height: 1 };
        ...
        let [slider_a, swatch_a, preview_a] =
            Layout::vertical([Constraint::Length(4), Constraint::Fill(1), Constraint::Length(9)]).areas(pad(right, 1, 0));

        let s_in = card(buf, slider_a, &th, "Primary hue override");
        if s_in.width > 12 {
            let track = Rect { x: s_in.x + 1, y: s_in.y, width: s_in.width.saturating_sub(12), height: 1 };
            self.slider = track;
            for i in 0..track.width {
                let hue = i as f32 / track.width.max(1) as f32 * 360.0;
                put(buf, track.x + i, track.y, "━", 1, st(Rgb::from_hsl(hue, 0.7, 0.5), th.background));
            }
            ...
            put(buf, tx, track.y, "●", 1, st(th.text, th.background).add_modifier(Modifier::BOLD));
            let label = format!("{:+.0}°", self.hue_shift);
            put(buf, track.right() + 2, track.y, &label, 9, st(th.text_muted, th.background));
            put(buf, s_in.x + 1, track.y + 1, "every derived role below follows the primary", s_in.width - 2, st(th.text_disabled, th.background));
        }
        ...
        for (i, (name, color)) in roles.iter().enumerate() {
            ...
            let block = Rect { x, y, width: cell_w - 1, height: 2 };
            fill(buf, block, *color);
            put(buf, x + 1, y, &color.to_hex_string(), cell_w - 2, st(color.text_on(0.8), *color));
            put(buf, x, y + 2, name, cell_w - 1, st(th.text_muted, th.background));
        }
        ...
        let x = p_in.x + 2;
        // 3D button
        let btn = Rect { x, y, width: 16, height: 3 };
        fill(buf, btn, th.primary);
        for bx in btn.left()..btn.right() {
            put(buf, bx, btn.y, "▔", 1, st(Rgb::shade(th.primary, 3), th.primary));
            put(buf, bx, btn.bottom() - 1, "▁", 1, st(Rgb::shade(th.primary, -3), th.primary));
        }
        put(buf, btn.x + 4, btn.y + 1, "Primary", 8, st(th.primary.text_on(0.9), th.primary).add_modifier(Modifier::BOLD));
        // input
        let inp = Rect { x: x + 18, y, width: 24, height: 3 };
        fill(buf, inp, th.focus_bg());
        Border::Tall.draw(buf, inp, th.border, th.focus_bg());
        put(buf, inp.x + 2, inp.y + 1, "typed text", 10, st(th.text, th.focus_bg()));
        put(buf, inp.x + 12, inp.y + 1, " ", 1, st(th.cursor_fg, th.cursor_bg));
        // text roles + gauge
        let tx = x + 44;
        if tx + 24 < p_in.right() {
            put(buf, tx, y, "text  ", 6, st(th.text, th.surface));
            put(buf, tx + 6, y, "muted  ", 7, st(th.text_muted, th.surface));
            put(buf, tx + 13, y, "disabled  ", 10, st(th.text_disabled, th.surface));
            put(buf, tx + 23, y, "link", 4, st(th.link, th.surface).add_modifier(Modifier::UNDERLINED));
            put(buf, tx, y + 1, "primary ", 8, st(th.text_primary, th.surface));
            put(buf, tx + 8, y + 1, "success ", 8, st(th.text_success, th.surface));
            put(buf, tx + 16, y + 1, "warning ", 8, st(th.text_warning, th.surface));
            put(buf, tx + 24, y + 1, "error", 5, st(th.text_error, th.surface));
            hbar(buf, tx, y + 2, 30.min(p_in.right().saturating_sub(tx)), 0.62, th.primary, th.panel);
        }
        // toast
        let ty = y + 4;
        if ty < p_in.bottom() {
            ...
            put(buf, sel.x + 1, ty, "selected text", sel.width, st(th.text, th.selection_bg));
            let hov = Rect { y: ty + 1, ..sel };
            fill(buf, hov, th.hover_bg);
            put(buf, hov.x + 1, ty + 1, "hovered row", hov.width, st(th.text, th.hover_bg));
        }
    }
        ...
        match ev {
            Event::Key(k) => {
                ...
                match k.code {
                    ...
                    KeyCode::Left => self.hue_shift -= if k.modifiers.contains(KeyModifiers::SHIFT) { 1.0 } else { 10.0 },
                    KeyCode::Right => self.hue_shift += if k.modifiers.contains(KeyModifiers::SHIFT) { 1.0 } else { 10.0 },
