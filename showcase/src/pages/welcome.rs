...
const FEATURES: [(&str, &str); 8] = [
    ("Design system", "Textual's colour math: one palette → 30 derived roles, 12 built-in themes"),
    ("40+ widgets", "buttons, switches, inputs, selects, tabs, trees, tables, charts, modals, toasts…"),
    ("Mouse & keyboard", "every widget handles clicks, hover, drag and wheel plus Textual key bindings"),
    ("Animation", "tweened switches, sliders, tabs, progress, toasts — 60 fps only while moving"),
    ("Overlays", "dropdowns, menus, tooltips, command palette, dialogs with dimmed backdrops"),
    ("Layout", "split panes, scroll views, panels, collapsibles, headers and footers"),
    ("Tiny API", "builder + State + Outcome: `Switch::new().render(area, buf, &mut state)`"),
    ("No bloat", "ratatui + unicode-width only; the runtime restores your terminal on panic"),
];
...
impl Page for WelcomePage {
    ...
    fn draw(&mut self, area: Rect, buf: &mut Buffer, ctx: &mut Ctx) {
        ...
        if show_banner {
            ...
            put_centered(buf, Rect { y, height: 1, ..inner }, "tuiforge", st(th.primary, th.background).add_modifier(Modifier::BOLD));
            y += 2;
        }
        put_centered(
            buf,
        ...
        for (i, (name, desc)) in FEATURES.iter().enumerate() {
            ...
            fill(buf, rect, bg);
            let border = if hovered { th.primary } else { th.border_blurred };
        ...
        if inner.height > 20 {
            put_centered(buf, Rect { y: sy, height: 1, ..inner }, &truncate(sample, inner.width as usize), st(th.text_disabled, th.background));
        }
    }
