...
impl Rgb {
    ...
    pub fn contrast_text(self) -> Rgb {
        if self.brightness() < 0.5 { WHITE } else { BLACK }
    }
    ...
    fn from_lab(l: f32, a: f32, b: f32) -> Rgb {
        ...
        let z = y - b / 200.0;
        let off = 16.0 / 116.0;
        let y = if y > 0.206_893 { y.powi(3) } else { (y - off) / 7.787 };
        let x = if x > 0.206_893 { 0.95047 * x.powi(3) } else { 0.122059 * (x - off) };
        let z = if z > 0.206_893 { 1.08883 * z.powi(3) } else { 0.139827 * (z - off) };
...
pub const BUILTIN: &[ThemeSpec] = &[
    theme!("textual-dark", dark = true, primary = 0x0178D4, secondary = 0x004578, accent = 0xffa62b,
        warning = 0xffa62b, error = 0xba3c5b, success = 0x4EBF71, foreground = 0xe0e0e0),
    theme!("textual-light", dark = false, primary = 0x004578, secondary = 0x0178D4, accent = 0xffa62b,
        warning = 0xffa62b, error = 0xba3c5b, success = 0x4EBF71, surface = 0xD8D8D8, panel = 0xD0D0D0,
        background = 0xE0E0E0),
    theme!("nord", dark = true, primary = 0x88C0D0, secondary = 0x81A1C1, accent = 0xB48EAD,
        foreground = 0xD8DEE9, background = 0x2E3440, success = 0xA3BE8C, warning = 0xEBCB8B,
        error = 0xBF616A, surface = 0x3B4252, panel = 0x434C5E),
    theme!("gruvbox", dark = true, primary = 0x85A598, secondary = 0xA89A85, warning = 0xfe8019,
        error = 0xfb4934, success = 0xb8bb26, accent = 0xfabd2f, foreground = 0xfbf1c7,
        background = 0x282828, surface = 0x3c3836, panel = 0x504945),
    theme!("catppuccin-mocha", dark = true, primary = 0xF5C2E7, secondary = 0xcba6f7, warning = 0xFAE3B0,
        error = 0xF28FAD, success = 0xABE9B3, accent = 0xfab387, foreground = 0xcdd6f4,
        background = 0x181825, surface = 0x313244, panel = 0x45475a),
    theme!("dracula", dark = true, primary = 0xBD93F9, secondary = 0x6272A4, warning = 0xFFB86C,
        error = 0xFF5555, success = 0x50FA7B, accent = 0xFF79C6, background = 0x282A36,
        surface = 0x2B2E3B, panel = 0x313442, foreground = 0xF8F8F2),
    theme!("tokyo-night", dark = true, primary = 0xBB9AF7, secondary = 0x7AA2F7, warning = 0xE0AF68,
        error = 0xF7768E, success = 0x9ECE6A, accent = 0xFF9E64, foreground = 0xa9b1d6,
        background = 0x1A1B26, surface = 0x24283B, panel = 0x414868),
    theme!("monokai", dark = true, primary = 0xAE81FF, secondary = 0xF92672, accent = 0x66D9EF,
        warning = 0xFD971F, error = 0xF92672, success = 0xA6E22E, foreground = 0xd6d6d6,
        background = 0x272822, surface = 0x2e2e2e, panel = 0x3E3D32),
    theme!("flexoki", dark = true, primary = 0x205EA6, secondary = 0x24837B, warning = 0xAD8301,
        error = 0xAF3029, success = 0x66800B, accent = 0x9B76C8, background = 0x100F0F,
        surface = 0x1C1B1A, panel = 0x282726, foreground = 0xFFFCF0),
    theme!("catppuccin-latte", dark = false, primary = 0x8839EF, secondary = 0xDC8A78, warning = 0xDF8E1D,
        error = 0xD20F39, success = 0x40A02B, accent = 0xFE640B, foreground = 0x4C4F69,
        background = 0xEFF1F5, surface = 0xE6E9EF, panel = 0xCCD0DA),
    theme!("solarized-light", dark = false, primary = 0x268bd2, secondary = 0x2aa198, warning = 0xcb4b16,
        error = 0xdc322f, success = 0x859900, accent = 0x6c71c4, foreground = 0x586e75,
        background = 0xfdf6e3, surface = 0xeee8d5, panel = 0xeee8d5),
    theme!("rose-pine", dark = true, primary = 0xc4a7e7, secondary = 0x31748f, warning = 0xf6c177,
        error = 0xeb6f92, success = 0x9ccfd8, accent = 0xebbcba, foreground = 0xe0def4,
        background = 0x191724, surface = 0x1f1d2e, panel = 0x26233a),
];
...
pub fn builtin(name: &str) -> Option<Theme> {
    BUILTIN.iter().find(|t| t.name == name).map(|t| Theme::resolve(t, None))
}
...
impl Theme {
    pub fn resolve(spec: &ThemeSpec, primary_override: Option<Rgb>) -> Theme {
        ...
        let panel = spec.panel.unwrap_or_else(|| {
            ...
            if spec.dark { p.blend(contrast, 0.04) } else { p }
        });
...
mod tests {
    ...
    fn hsl_roundtrip_and_parse() {
        ...
        assert_eq!(Rgb::parse("0178d4"), Some(c.blend(c, 0.0)).map(|_| Rgb::hex(0x0178D4)));
