...
impl CollapsibleState {
    pub fn new(open: bool) -> Self {
        Self {
            open,
            anim: if open { Tween::new(1.0) } else { Tween::new(0.0) },
            hit: HitBox::default(),
    ...
    pub fn toggle(&mut self, now: Instant) {
        ...
        if self.duration.is_zero() {
            ...
            self.anim.go_with(target, now, self.duration, Easing::InOutCubic);
        }
    }
...
impl Collapsible {
    ...
    pub fn header_style(mut self, h: CollapsibleHeader) -> Self {
        self.header_style = h; self
    }

    pub fn duration(mut self, ms: u64) -> Self {
        self.duration = Duration::from_millis(ms); self
    }
    ...
    pub fn render_with<F>(self, area: Rect, buf: &mut Buffer, state: &mut CollapsibleState, content_height: u16, mut draw_fn: F) -> u16
    where F: FnMut(Rect, &mut Buffer)
    {
        ...
        // Header
        let marker = if state.open { self.markers.1 } else { self.markers.0 };
        ...
        let style = if self.focused || state.open { st(header_fg, header_bg).add_modifier(Modifier::BOLD) } else { st(header_fg, header_bg) };
        ...
        if self.header_style == CollapsibleHeader::Underline && area.height > 1 {
            for x in area.x..area.right() {
                if let Some(c) = buf.cell_mut((x, area.y + 1)) {
                    c.set_symbol("─").set_fg(th.border_blurred.color()).set_bg(th.background.color());
                }
            }
        }
...
impl AccordionState {
    pub fn new(count: usize, exclusive: bool) -> Self {
        Self { states: (0..count).map(|_| CollapsibleState::new(false)).collect(), exclusive, focus: 0 }
    }

    /// Toggle item `i`, closing others if exclusive.
    pub fn toggle(&mut self, i: usize, now: Instant) {
        if i >= self.states.len() { return; }
...
impl Accordion {
    pub fn new() -> Self {
        Self { exclusive: false, gap: 0, titles: Vec::new(), header_style: CollapsibleHeader::Plain, border: None, focused: false, theme: None }
    }
    ...
    pub fn render_with<F>(self, area: Rect, buf: &mut Buffer, state: &mut AccordionState, heights: &[u16], mut draw_fn: F)
    where
        F: FnMut(usize, Rect, &mut Buffer),
    {
        ...
        for (i, (item_state, &content_h)) in state.states.iter_mut().zip(heights.iter()).enumerate() {
            ...
            let item_area = Rect { x: area.x, y, width: area.width, height: area.bottom().saturating_sub(y) };
            let title = self.titles.get(i).cloned().unwrap_or_else(|| format!("Item {}", i + 1));
            let mut c = Collapsible::new().title(title).header_style(self.header_style).focused(self.focused && state.focus == i).theme(&th);
