...
use crate::draw::{self, fill, put, put_centered, put_right, st};
...
impl MinSize for AppHeader {
    /// Compact header is 1 row, tall header is 3 rows.
    fn min_size(&self) -> (u16, u16) {
        if self.tall { (8, 3) } else { (8, 1) }
    }
}
